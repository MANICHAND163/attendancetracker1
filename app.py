...
def extract_attendance():
    df = pd.read_csv(f'Attendance/Attendance-{datetoday}.csv', dtype=str)
    names = df['Name']
    rolls = df['Roll']
    times = df['Time']
    l = len(df)
...
def add_attendance(name):
    # Expect labels like "username_roll"; split from the right to allow underscores in names
    ...
    df = pd.read_csv(f'Attendance/Attendance-{datetoday}.csv', dtype=str)
    existing_rolls = set(df['Roll'].astype(str).tolist())
...
def register():
    ...
    if request.method == 'POST':
        ...
        i, j = 0, 0
        # Use DirectShow backend on Windows for faster startup
